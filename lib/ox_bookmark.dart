...
class OXBookmarkPage extends StatefulWidget {
  @override
  _OXBookmarkPageState createState() => _OXBookmarkPageState();
}

class _OXBookmarkPageState extends State<OXBookmarkPage> {
  List<Map<String, dynamic>> bookmarkedQuestions = [];
  Map<String, String> localSelectedOptions = {}; // 고유 키 사용
  Map<String, bool> localShowDescription = {}; // 고유 키 사용
  bool isLoading = true;
  String errorMessage = '';
  
  int _currentIndex = 2; // 하단 탭(북마크)이 기본 인덱스
  ...
  void _navigateToPage(int index) {
    switch (index) {
      case 0:
        ...
        break;
      case 1:
        // OX 오답노트 아이콘 클릭 시 → 전면광고 후 OXWrongAnswerPage 이동
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => OXWrongAnswerPage()),
        );
        break;
      case 2:
        // 현재 북마크 페이지이므로 아무 것도 안 함
        break;
    }
  }
  ...
  /// bookmarkedQuestions 로드
  Future<void> fetchOXBookmarks() async {
    setState(() {
      isLoading = true;
      errorMessage = '';
    });
    ...
    try {
      ...
      final allBookmarkList = prefs.getStringList('ox_savedQuestions') ?? [];
      
      // "OX|NN" 형태만 추출
      final oxBookmarks = allBookmarkList.where((item) => item.startsWith('OX|')).toList();
      
      if (oxBookmarks.isEmpty) {
        setState(() {
          isLoading = false;
          bookmarkedQuestions = [];
        });
      ...
      for (String item in oxBookmarks) {
        try {
          final parts = item.split('|');
          if (parts.length >= 2) {
            final uniqueKey = parts.sublist(1).join('|');
            final jsonString = prefs.getString('ox_bookmark_data_$uniqueKey');
            if (jsonString != null) {
          ...
          print('Error processing OX bookmark item $item: $e');
        }
      }

      setState(() {
        bookmarkedQuestions = tempList;
        isLoading = false;
      });
    } catch (e) {
      print('Error in fetchOXBookmarks: $e');
      setState(() {
        isLoading = false;
        errorMessage = 'OX 북마크를 불러오는 중 오류가 발생했습니다.';
      });
    }
  }

  /// 옵션 탭
  void handleOptionTapInBookmarkPage({
  ...
  Widget _buildEmptyBookmarkView(bool isDarkMode) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.bookmark_border, size: 64, color: isDarkMode ? Colors.grey : Colors.grey[400]),
          SizedBox(height: 16),
          Text(
            '저장된 OX 북마크 문제가 없습니다.',
            style: TextStyle(
