...
class _WrongAnswerPageState extends State<WrongAnswerPage> with TickerProviderStateMixin {
  ...
  Map<String, bool> savedQuestions = {}; // 오답노트에서도 북마크 상태 관리를 위함
  ...
  void initState() {
    super.initState();
    futureQuestions = fetchWrongAnswers();
    loadSavedQuestionStatus(); // 북마크 상태 로드
    if (!adsRemovedGlobal) {
      _loadInterstitialAd();
    }
  ...
  Future<void> loadSavedQuestionStatus() async {
    ...
    if (mounted) {
      setState(() {
        savedQuestions = currentSavedStatus;
      });
    }
  }
  
Future<List<Map<String, dynamic>>> fetchWrongAnswers() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> wrongAnswerList = prefs.getStringList('wrongAnswers_${widget.round}') ?? [];
    ...
    for (String item in wrongAnswerList) {
      ...
      if (parts.length == 2) {
        ...
        try {
          ...
          print('Invalid questionId format in $item: $e');
          continue;
        }
        ...
        int? roundValue = reverseRoundMapping.entries
            ...
            .key;
        if (roundValue == -1) {
          print('Invalid round name: $roundName');
          continue;
        }
        ...
        if (questionData != null) {
          ...
          question['saveKey'] = '$roundName|$questionId';
  ...
  Future<void> handleSaveTap(String saveKey) async {
    bool isCurrentlySaved = savedQuestions[saveKey] ?? false;
    bool newSaveStatus = !isCurrentlySaved;

    setState(() {
      savedQuestions[saveKey] = newSaveStatus;
    });
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text('오답노트', style: TextStyle(color: isDarkMode ? Colors.white : Colors.black87, fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 0.3)),
  ...
  void _navigateToPage(int index) {
    ...
    switch (index) {
      case 0:
        ...
        break;
      case 1: // 현재 오답노트 페이지
        futureQuestions = fetchWrongAnswers(); // 목록 새로고침
        return;
      case 2:
        page = BookmarkPage(round: widget.round, dbPath: widget.dbPath);
        routeName = '/bookmark';
        break;
      default:
        return;
...
Widget _buildModernQuestionWidget(dynamic questionData, bool isDarkMode) {
  ...
  if (questionData is String && questionData.isNotEmpty) {
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Image.memory(
          questionData,
          fit: BoxFit.contain, // 이미지가 잘리지 않도록 contain으로 설정
        ),
      ),
    );
    // ▲▲▲▲▲ 여기까지 추가/수정되었습니다 ▲▲▲▲▲
  ...
  Widget _buildOptionRow(String saveKey, dynamic optData, String letter, String correctStr, String? selectedOpt, Map<String, dynamic> question, bool isDarkMode, {bool isFirst = false, bool isLast = false}) {
    ...
    if (isAnswered) {
      ...
      } else if (isCorrectChoice) { // 사용자가 선택하지 않았지만 정답인 옵션
                        ...
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 16),
                                Text('저장된 오답 문제가 없습니다', style: TextStyle(color: isDarkMode ? Colors.white : Colors.black87, fontSize: 16)),
                                SizedBox(height: 8),
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final isBookmarked = savedQuestions[saveKey] ?? false;
                                              ...
                                              children: [
                                                ...
                                                if (category != null && category.isNotEmpty)
                                                  Container(
                                                    ...
                                                    decoration: BoxDecoration(
                                                      // 오답노트에서는 카테고리 태그 색상을 다르게 표시
                                                      color: Color(0xFFF44336).withOpacity(0.15), // MODIFIED
                                                      borderRadius: BorderRadius.circular(8), // MODIFIED
                                                    ),
                                                    child: Text(category, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Color(0xFFF44336))), // MODIFIED
                                                  ),
                                              ],
                                            ),
                                          ),
                                          // 오답노트에서는 북마크 아이콘 추가 (필요시)
                                           GestureDetector(
                                            onTap: () => handleSaveTap(saveKey),
                                            child: Container(
                                              padding: EdgeInsets.all(7),
                                              decoration: BoxDecoration(
                                                color: isBookmarked ? Color(0xFF2196F3).withOpacity(0.15) : (isDarkMode ? Colors.white.withOpacity(0.1) : Colors.grey.withOpacity(0.1)),
                                                borderRadius: BorderRadius.circular(8),
                                              ),
                                              child: Icon(
